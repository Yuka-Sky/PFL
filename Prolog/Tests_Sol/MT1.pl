...
%movie(MovieTitle, YearofRelease, SagaID, Duration, IMDBScore, Director, Composer, Cast)

same_saga(Movie1, Movie2):-
    movie(Movie1, _, Saga, _, _, _, _, _),
    movie(Movie2, _, Saga, _, _, _, _, _),
    Movie1 \= Movie2.

movie_from_saga(Movie, Saga):-
    saga(ID, Saga, _, _),
    movie(Movie, _, ID, _, _, _, _, _).

saga_best_movie(Saga, Movie):-
    saga(ID, Saga, _, _),
    movie(Movie, _, ID, _, Score, _, _, _),
    \+ (movie(_, _, ID1, _, OtherScore, _, _, _), ID =:= ID1, OtherScore > Score).

add_movie_to_saga(Saga, Movie, Year, Duration, Score, Director, Composer, Cast):-
    saga(ID, Saga, N, Creator),
    \+ movie(Movie, _, Saga, _, _, _, _, _),
    assert(movie(Movie, Year, Saga, Duration, Score, Director, Composer, Cast)),
    N1 is N + 1,
    retract(saga(ID, Saga, N, Creator)),
    assert(saga(ID, Saga, N1, Creator)).


movies_from_saga(Saga, Movies):-
    saga(_, Saga, N, _),
    collect(Saga, N, MoviesList),
    orderer(MoviesList, N, Movies).

collect(_, 0, []). [(X, Y)|T]
collect(Saga, N, [Movie-Year|T]):-
    movie(Movie, Year, Saga, _, _, _, _, _),
    N1 is N-1,
    collect(Saga, N1, T).

orderer(MoviesList, Movies) :-
    sort(1, @=<, MoviesList, SortedPairs),
    extract_titles(SortedPairs, Movies).

extract_titles([], []).
extract_titles([Title-_ | Pairs], [Title | Titles]) :-
    extract_titles(Pairs, Titles).

saga_cast(Saga, Cast):-
    saga(ID, Saga, _, _), 
    findall(Actor, (movie(_, _, ID, _, _, _, _, CastList), member(Actor, CastList)), AllActors),
    sort(AllActors, Cast).
...
composed_for(Composer, Movie):-
    movie(Movie, _, _, _, _, _, Composer, _).


composer_rating(Composer, AvgScore) :-
    findall(Score, movie(_, _, _, _, Score, Composer, _, _), Scores),  % Collect all scores for the composer
    length(Scores, Count),  % Count the number of movies
    Count > 0,  % Ensure that the composer has worked on at least one movie
    sum_list(Scores, TotalScore),  % Sum up all the scores
    AvgScore is TotalScore / Count.  % Calculate the average score


most_successful_composer(Composer1, Composer2, Composer):-
